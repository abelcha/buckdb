import { MaterializedResult, MState, MetaModel, NestedKeyOf, StrictCollection } from "./build.types";
import { DuckdbCon } from "./utils";
...
export interface CopyTo<AvailableFields extends MetaModel = MetaModel, Options extends GenericCopyOptions<AvailableFields> = GenericCopyOptions<AvailableFields>> {
  copyTo: ((destination: csvFile, options?: CsvCopyOptions & Options) => ReturnValue)
 ...
 * ```
 */
export function copy<S extends MState, C extends StrictCollection[]>(
  source: MaterializedResult<S, C>
): CopyResult<S, C> {
  return {
