    ...
    for (const segment of segments) {
        ...
        if (segment.startsWith('"')) {
            ...
            for (let j = 1; j < segment.length; j++) {
                if (segment[j] === '"' && segment[j - 1] !== '\\') { nameEndQuoteIndex = j; break; }
            }
            if (nameEndQuoteIndex !== -1) { name = segment.substring(0, nameEndQuoteIndex + 1); i = nameEndQuoteIndex + 1; }
            ...
            if (firstSpaceIndex !== -1) { name = segment.substring(0, firstSpaceIndex).trim(); i = firstSpaceIndex; }
            else { console.warn(`No clear name/type split: "${segment}"`); if (!segment.match(/^([A-Z_]+(\(.*\)|\[\])?|STRUCT\(|JSON|LIST\()/i) && !segment.endsWith('[]')) { name = segment; type = 'UNKNOWN'; } else continue; }
        }
...
function parseBaseTypeToIntermediateValue(baseType: string): IntermediateColumnValue {
    ...
    if (upperBaseType.startsWith('STRUCT(') && baseType.endsWith(')')) {
        ...
        if (openParenIndex !== -1) {
            ...
            } else { console.warn(`No matching parenthesis for STRUCT: ${baseType}`); return mapTypes(baseType); }
