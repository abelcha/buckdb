...
type RetCon1<S extends string, F = null> = F extends null ? `${S}()` : `${S}(${SerializeValue<F>})`
type RetCon2<S extends string, F1, F2> = `${S}(${SerializeValue<F1>},${SerializeValue<F2>})`
type RetCon3<S extends string, F1, F2, F3> = `${S}(${SerializeValue<F1>},${SerializeValue<F2>},${SerializeValue<F3>})`
...
export interface TFMetadata {
    ...
    duckdb_table_sample<T extends string>(col0: T): RetCon1<'duckdb_table_sample', T>;
...
export interface TFBase {
    ...
    arrow_scan_dumb<T1, T2, T3>(pointer1: T1, pointer2: T2, pointer3: T3): RetCon3<'arrow_scan_dumb', T1, T2, T3>;
    ...
    query<T>(sql: string): RetCon1<'query', T>;
    ...
    query_table<T>(tables: string, validate?: boolean): RetCon1<'query_table', T>;
    ...
    repeat<T>(value: T, count: number): RetCon1<'repeat', T>;
    /** @description Repeats a row of values numRows times. @description @param numRows Number of rows to generate */
    repeat_row<T extends any[]>(numRows: number, ...values: T): RetCon1<'repeat_row', T>;
    ...
    summary<T>(table: T): RetCon1<'summary', T>;
    ...
    test_vector_types<T1, T2>(vector: T1[], allFlat: boolean, ...args: T2[]): RetCon2<'test_vector_types', T1, T2>;
    /** @description Unnests an array into individual rows. @description @param array Array value to unnest */
    unnest<T>(array: T[]): RetCon1<'unnest', T>;
...
export interface TFExtensions {
    ...
    arrow_scan<T1, T2, T3>(pointer1: T1, pointer2: T2, pointer3: T3): RetCon3<'arrow_scan', T1, T2, T3>;
    /** get_ui_url() @description Returns the URL of the DuckDB web UI for this connection. */
    get_ui_url(): RetCon<'get_ui_url'>;
    ...
    sqlite_query<T1, T2>(database: T1, query: T2): RetCon2<'sqlite_query', T1, T2>;
    ...
    sqlite_scan<T1, T2>(database: T1, table: T2): RetCon2<'sqlite_scan', T1, T2>;
