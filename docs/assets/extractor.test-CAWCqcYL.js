const e=`import { it, describe, expect } from 'bun:test'
import { Extracted, extractAssignations, extractReconciledCalls, extractSpecialCalls, getRessourcesExtracts, getChainsTargets, getRessources, getTargets, getTargetOps, getRessourceChain, getBuckInstanceInit } from './extractor'
...
it('test getTargetOps', () => {
    const c = \`
    from('starbase/*.parquet', 'a')
    .crossJoin('b')
    .leftJoin('ccc/cc').on('u')
    .select()
    .limit(100)
    \`
    const g = extractReconciledCalls(c)
    const r = getTargetOps(g)
    expect(r).toEqual(["starbase/*.parquet", "ccc/cc"])
})

it('test getRessources', () => {
    const ext = extractReconciledCalls(testCode)[0]
    const rrr = getRessourcesExtracts(ext)
    expect(rrr).toEqual(['res1', 'param1'])
})

it('test getRessourceChain', () => {
    const ext = extractReconciledCalls(\`
    Buck('s3://lol/toto.db', {s3_whatever: 'xx'}).settings({max_threads:1}).loadExtensions('spatial').from('zzz').join('ooo').on('ddd').select().execute()
    \`)
    const rrr = getBuckInstanceInit(ext[0])
    expect(rrr).toEqual(["Buck('s3://lol/toto.db',{s3_whatever: 'xx'})", "settings({max_threads:1})", "loadExtensions('spatial')"])
    // .map(e =>e.)
    // expect(rrr).toEqual(['res1', 'param1'])
})`;export{e as default};
