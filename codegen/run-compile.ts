...
// const duckdb_extensions = await Buck().from('duckdb_extensions()').select(e => e.extension_name).execute()
const duckdb_extensions = ["arrow", "aws", "azure", "delta", "excel", "fts", "h3", "httpfs", "iceberg", "inet", "spatial", "sqlite_scanner", "ui"]
const allextensions = [...duckdb_extensions, ...community_extensions]
...
async function getMergedResults() {
    ...
    const groups = groupBy(results, e =>
        [e.function_name, e.function_type === 'scalar' && e.parameter_types[0], mapTypesProps(e.return_type).inferredTo].join('-')
    )
...
const genArgs = (args: IFns['args'], varargs: string) => {
    return args.map(x => {
        ...
        // console.log(x.ptypes, x.dtypes.map((e) => mapTypesProps(e, true)))
        // console.log('-----------')
...
global.renderMethods = (opts: { type: string, match: (() => boolean), typeMap: Record<string, any>, slice: number, override: string[] }) => {
    ...
    // console.log()
    const funcs = uniqBy(
        typeof opts.match === 'function' ? _fns.filter(opts.match) :
...
const fn = eval(" `" + body + "`")
await Bun.write('.buck/types.ts', fn)
// console.log({ _ })
console.log(new Date().toISOString(), 'Generated .buck/types.ts')
_ && await Bun.$`dprint fmt .buck/types.ts`;
// console.log({ total })
