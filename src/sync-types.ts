...
import { database, from } from '../buckdb';

const anyFuncs = Object.entries(PatternMatchers).map(([k, e]) => ({
    function_name: k,
...
for (let i in resp3) {
    ...
    const dest =  TypeProps[tmap[i.toUpperCase()] || 'DAny']
    // const wildcard = '`${string}(${number}, ${number})` | `${string}[]` |  `[${string}]`'
    const n = `D${i.replace('null', 'ANY')}_NATIVE`
    acsheaders += (`export type ${n} = ${enms};\n`)
    funcas += (`as(destype: ${n}, b?: DNumericable, c?: DNumericable): ${dest.field};\n`)
    funcomp += (`as(destype: ${n}, b?: DNumericable, c?: DNumericable): ${dest?.man || dest?.field};\n`)

}
console.log(acsheaders)
console.log('-------')
console.log(funcas)
// process.exit()

// const asFuncs = resp.flatMap(e => {
//     const tmap = {
//         BOOLEAN: 'DBool',
//         NUMERIC: 'DNumeric',
//         STRING: 'DVarchar',
...
//         parameter_types: ['ANY', `D$}{}`],
//         return_type: tmap[e.type_category?.toUpperCase()] || 'DOther',
//         description: 'CASTSSS',
//         examples: ['castas("Decimal")'],
//         varargs: null,
//     }
// })
// console.log(asFuncs)
...
const generateSettings = async () => {
    ...
    const extts = await db.from('duckdb_extensions()')
        ...
        .execute()
    console.log({ extts })
    db.load("arrow", "aws", "azure", "delta", "excel", "fts", "h3", "httpfs", "iceberg", "inet", "spatial", "sqlite_scanner", "ui")
    // console.log('keekek')
...
if (import.meta.main) {
    const main = async () => {
        ...
        // console.log({ rres })
        const db = from('duckdb_functions()')
        ...
        const query = db
            ...
            .where(e => e.function_name.similarTo(/[a-z]\w+/) && e.function_name.notLike('icu_collate%'))
